...
def get_dependencies(package_name, max_depth):
    ...
    while queue:
        ...
        if result.returncode != 0:
            print(f"Error fetching dependencies for {current_package}: {result.stderr}")
            continue
        ...
        try:
            ...
            print(f"Error parsing dependencies for {current_package}: {e}")
...
def main(config_path):
    ...
    print(f"Graph saved to {config['output_image_path']}")
